...
--[[----------
	Variables
--------------]]
local UnitScale = 1.193
--[[local unitframesPartyPosition = {
	relAnchor = "TOPLEFT",
	offSetX = 10,
	offSetY = -200,
},]]
...
--[[----------------
	Frame Creation
--------------------]]
local AddonFrame = CreateFrame("Frame", addonName, UIParent)
...
function Addon:ADDON_LOADED(addon)
    if addon == addonName then
		...
		self:FontStyle()
		self:FontColor()
		self:CenterLevelText()
		self:MainFramesScale()
		self:PartyFramesScale()
		self:ArenaFramesScale()
		self:BossFramesScale()
		self:CastingbarScale()
...
function Addon:CenterLevelText()
	-- PlayerFrame
...
function Addon:MainFramesScale()
...
function Addon:PartyFramesScale()
	for i = 1, MAX_PARTY_MEMBERS do
		_G["PartyMemberFrame"..i]:SetScale(UnitScale)
	end
end

function Addon:ArenaFramesScale()
...
function Addon:BossFramesScale()
	for i = 1, MAX_BOSS_FRAMES do
		_G["Boss"..i.."TargetFrame"]:SetScale(UnitScale)
	end
end

function Addon:CastingbarScale()
	-- Player Castbar
